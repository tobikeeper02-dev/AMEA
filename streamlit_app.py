...
try:
    ...
    st.error(
        "Failed to load the AMEA modules. Ensure dependencies are installed with `pip install -r requirements.txt` "
        "and that you are running Streamlit from the project root.\n\n"
        f"Details: {exc}"
    )
    st.stop()
