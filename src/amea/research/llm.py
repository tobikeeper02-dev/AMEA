"""Lightweight ChatGPT helper for AMEA.

All requests are made on demand; there is no cached data or baked-in output.
The helpers here intentionally avoid complex structures to reduce the chance of
syntax or indentation errors interfering with imports.
"""
    ...
    def ensure_key(self) -> None:
        if not self.api_key:
            raise ChatGPTNotConfiguredError(
                "OpenAI API key is required. Provide it via the sidebar or OPENAI_API_KEY."
            )
...
class ChatGPTClient:
    ...
    def complete(self, *, prompt: str, system: str, json_mode: bool = False) -> str:
        kwargs = self._build_kwargs(json_mode=json_mode)
        kwargs["messages"] = [
    ...
    def healthcheck(self) -> str:
        return self.complete(
            prompt="Confirm in one short sentence that the AMEA ChatGPT endpoint is reachable.",
            system="You briefly acknowledge API connectivity.",
        )
